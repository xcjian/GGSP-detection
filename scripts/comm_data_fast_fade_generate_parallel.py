...
parser.add_argument('--repeat_times', type=int, default=100,
...
repeat_times = np.array(list(range(args.repeat_times)))

#--------------------------------
# generate the p-value processes
#--------------------------------

dat_folder = paths.get_path_to_dat(FD_SCEN)
res_folder = paths.get_path_to_res(FD_SCEN)

def generate_pval(noise_std, repeat_idx):

    noise_idx = np.where(noise_levels == noise_std)[0][0]

    dat_dir = os.path.join(dat_folder, 'fast_fade_data', f'noise_{noise_idx}')
    res_dir = os.path.join(res_folder, 'fast_fade_res', f'noise_{noise_idx}')
...
pairs = list(itertools.product(noise_levels, repeat_times))
# pairs = [(0.25, 1), (0.5, 0)]

# Parallel processing
with Pool(processes=num_wrk) as pool:  # Adjust processes as needed
    pool.starmap(generate_pval, pairs)
