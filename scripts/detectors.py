...
class DetectionResult(object):
    ...
    def __init__(self, nam, tar, thr, r_tru, r_det, cpl=True,
        ...
        if np.array(r_tru).shape == np.array(r_det).shape:
            ...
            #calculate the FDR over all runs
            self.fdr = np.sum(self.v)/(np.sum(self.r_det) + (np.sum(self.r_det) == 0) * 1)
            # self.pow = np.mean(self.tdp, axis=0)
            #calculate the power over all runs
            self.pow = np.sum(self.s)/(np.sum(self.r_tru) + (np.sum(self.r_tru) == 0) * 1)
...
def apply_lfdr_detection_GGSP(lfdrs, r_tru, alp_vec, name, sen):
    """Obtain detection results with lfdrs, under GGSP.
    ...
    for idx in np.arange(0, alp_vec.size, 1):
        det_res.append(DetectionResult('lfdrs ' + name, 'fdr', alp_vec[idx],
        r_tru, bh_loc_bayes_GGSP(lfdrs, alp_vec[idx]), sen=sen))
    return det_res

def apply_lfdr_detection_BUM_GGSP_thre(lfdrs, r_tru, threshold, name, sen):
    """Obtain detection results with lfdrs, under BUM_GGSP, using any thresold.
        ...
        """
    det_res = []
    for idx in np.arange(0, threshold.size, 1):
        threshold_temp = threshold[idx]
        det_res.append(DetectionResult('lfdrs ' + name, 'fdr', threshold_temp,
                                       r_tru, bh_loc_bayes_thre(lfdrs, threshold_temp), sen=sen))
    return det_res
...
def bh_loc_bayes_GGSP(lfdr, alp):
    """
        Perform the local fdr procedure under the GGSP framework.

        Finds the largest region in joint domain for which the Bayesian FDR (summed and normalized
        local fdrs) is smaller than the nominal level.

        Parameters
        ----------
        lfdr : array
            The MC x N matrix of local fdrs. MC here also refers to the number of time steps.
                - MC:   # of MC runs
                - N:    # of tested hypotheses
        alp : float
            The FDR nominal level.
        ...
        @Xingchao Jian
        """
    lfdr_vec = lfdr.reshape(1, lfdr.size)
    r_vec = bh_loc_bayes(lfdr_vec, alp)
    r = r_vec.reshape(lfdr.shape)

    return r

def bh_loc_bayes_thre(lfdr, level):
    """
        Perform the local fdr procedure.

        Finds the region for which the local fdr is smaller than the level.
