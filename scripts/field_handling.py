...
class RadioSpatialField(SpatialField):
    ...
    def __init__(self, scen, dim, n_MC, n_tr_sam, n_obs_sam, noise_std):
        if len(dim) != 2:
            ...
            super().__init__(scen, dim, n_MC, n_tr_sam, n_obs_sam, noise_std)
        self.res_fd()
        ...
        """Remarks on the specification of the alternative events:
            ...
            X: num MC runs x num of grid points
              "The radio field level matrix"
               ...
               grid point level.
            X_sf: num MC runs x num of grid points x num of sources
            "The signal matrix after shadow fading, before fast fading"
             Captures the observed field level at each individual spatial
             unit before fast fading, which is added only on the
             grid point level.
            """
    ...
    def pla_tra(self, fd_scen, dat_path):
        ...
        self.X_sf = np.zeros((self.n_MC, self.n, n_src))
        ...
        with warnings.catch_warnings():
            ...
            alt_ind = np.zeros((self.n_MC, self.n_src), dtype=object)

        sf_collection = np.zeros((self.n_MC, self.n_src, self.n))
        ...
        if ran_pre:
            ...
            for mc in np.arange(0, self.n_MC, 1):
                ...
                [[cov[1,1], -cov[0,1]],[-cov[1,0], cov[0,0]]])
                    ...
                    else:#shadow fading, with temporal correlation
                        if mc == 0: # initialize with a random realization
                            sf =(self.sha_fa(
                                x_rge, y_rge, x_crd[cen_ev[mc, k]],
                                y_crd[cen_ev[mc, k]], sig_s).reshape(self.n))

                            sf_collection[mc, k, :] = sf
                        else: # set the shadow fading as constant
                            sf = sf_collection[mc - 1, k]
                            sf_collection[mc, k, :] = sf

                        if fast_fa['exist']:  # fast fading
                            sfded_sig = 10 ** ((sig + sf) / 10)
                            self.X_sf[mc, :, k] = sfded_sig
                            fast_fad_sig = fast_fading(sfded_sig, fast_fa)
                            alt_ind[mc, k] = np.where(fast_fad_sig >= s_r_min)[0]
                            if add_tra:  # the received signals add up
                                self.X[mc, :] += fast_fad_sig
                            else:  # the received signals do not add up
                                self.X[mc, alt_ind[mc, k]] = np.max(
                                    (fast_fad_sig[alt_ind[mc, k]],
                ...
                self.X[mc, np.where(np.abs(self.X[mc, :])<s_r_min)[0]] = 0
            ...
            if np.sum(n_sen == self.n) > 0:
                ...
                for sen_type in np.arange(num_sen_types):
                    ...
                    if var_sen_loc[sen_type]:
                        ...
                        if sen_hom[sen_type]:
                            # manually fix the sensor locations at the 0-th mc.
                            random_sens_locs = np.random.choice(np.setdiff1d(np.arange(np.prod(self.dim)), all_indeces[0, :]), n_sen[sen_type], replace=False).astype(int)
                            sen_idx[sen_type] = np.tile(random_sens_locs, (self.n_MC, 1))
                            for mc in np.arange(self.n_MC):
                                '''
...
class GridPoint(object):
    ...
    def __init__(self, sgl_id, crd, h, n_MC, n_tr_sam, n_obs_sam, noise_std):
        ...
        self.X_0 = stats.norm(0, noise_std) # Observations under H0 zero-mean Gaussian
    ...
    def hyp_tes(self):
        ...
        for mc in np.arange(self.n_MC):
            self.tau[mc] = np.sum(
                np.abs(self.x_obs[mc, 0:self.n_obs_sam_used[mc]])**2) / self.X_0.std()**2
...
def dynamic_centers(init_pos, n_time_steps, grid_speed, x_ran, y_ran):
    ...
    Initial positions of the transmitters. size = n_src x 2.
    each element is the index of the transmitter, take values in {1, ..., n_grid_points}.

    n_time_steps : int
    Number of time steps for which the dynamic centers are to be created.
    ...
    Returns
    -------
    The dynamic positions of the transmitters.
    """

    n_dynamic_points = init_pos.shape[0]
    n_space_dim = init_pos.shape[1]
    dynamic_pos = np.zeros((n_time_steps, n_dynamic_points, n_space_dim))
    x_min = x_ran[0]
    x_max = x_ran[1]
    y_min = y_ran[0]
    y_max = y_ran[1]

    dynamic_pos[0, :, :] = init_pos
    for t in range(1, n_time_steps):
        ...
        for k in range(n_dynamic_points):
            ...
            if direction == 0:
                ...
                dynamic_pos[t, k, :] = dynamic_pos[t - 1, k, :] + np.array([-grid_speed, 0])
            else:
                dynamic_pos[t, k, :] = dynamic_pos[t - 1, k, :] + np.array([grid_speed, 0])
            # check if the new position is out of bound
            if dynamic_pos[t, k, 0] < x_min:
                dynamic_pos[t, k, 0] = x_min + grid_speed
            elif dynamic_pos[t, k, 0] > x_max:
                dynamic_pos[t, k, 0] = x_max - grid_speed
            if dynamic_pos[t, k, 1] < y_min:
                dynamic_pos[t, k, 1] = y_min + grid_speed
            elif dynamic_pos[t, k, 1] > y_max:
                dynamic_pos[t, k, 1] = y_max - grid_speed

    dynamic_pos = dynamic_pos.astype(int)
    x_pos = dynamic_pos[:, :, 0]
    y_pos = dynamic_pos[:, :, 1]
    return x_pos, y_pos

def dynamic_centers_with_rebound(init_pos, n_time_steps, grid_speed, x_ran, y_ran):
    """Create dynamic centers for transmitters with rebound behavior.
    ...
    dynamic_pos[0, :, :] = init_pos
    directions = np.random.randint(0, 4, n_dynamic_points)

    for t in range(1, n_time_steps):
        for k in range(n_dynamic_points):
            if directions[k] == 0:
                new_pos = dynamic_pos[t - 1, k, :] + np.array([0, grid_speed])
            elif directions[k] == 1:
                new_pos = dynamic_pos[t - 1, k, :] + np.array([0, -grid_speed])
            elif directions[k] == 2:
                new_pos = dynamic_pos[t - 1, k, :] + np.array([-grid_speed, 0])
            else:
                new_pos = dynamic_pos[t - 1, k, :] + np.array([grid_speed, 0])

            if new_pos[0] < x_min:
                new_pos[0] = x_min + (x_min - new_pos[0])
                directions[k] = 3  # Change direction to right
            elif new_pos[0] > x_max:
                new_pos[0] = x_max - (new_pos[0] - x_max)
                directions[k] = 2  # Change direction to left
            if new_pos[1] < y_min:
                new_pos[1] = y_min + (y_min - new_pos[1])
                directions[k] = 0  # Change direction to up
            elif new_pos[1] > y_max:
                new_pos[1] = y_max - (new_pos[1] - y_max)
                directions[k] = 1  # Change direction to down

            dynamic_pos[t, k, :] = new_pos
...
def fast_fading(sig, fast_fa):

    fast_fa_type = fast_fa['type']
    if fast_fa_type == 'Rayleigh':  # Rayleigh fading
        param_scale = fast_fa['param']

        ffd = np.random.rayleigh(param_scale, len(sig))
        sig = sig * ffd
    elif fast_fa_type == 'Rician':  # Rician fading
        param_K = fast_fa['param']
        param_Omega = sig ** 2
        param_b = np.sqrt(2 * param_K)
        param_scale = np.sqrt(param_Omega / (2 * (param_K + 1)))

        ffd = stats.rice.rvs(param_b, scale=param_scale, size=len(sig))
        sig = ffd
    else:
        raise ValueError('Unknown fast fading type')
