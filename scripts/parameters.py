...
def get_par_fd_scen(fd_scen, dat_path):
    ...
    float
        The desired proportion of grid points where H0 holds (between 0 and 1).
    logical
    ...
    try:
        ...
        if fd_scen == 'scA_TSIPN' or fd_scen == 'scA_CISS':
            ...
            sha_fa = True
            # Do we use a field with fast fading?
            fast_fa = {'exist': True, 'type': 'Rician', 'param': 20}
            ...
            # Do we use a field with fast fading?
            fast_fa = {'exist': True, 'type': 'Rician', 'param': 20}
            # Here the parameter is K, the Rician factor. See wikipedia (https://en.wikipedia.org/wiki/Rician_fading).
            prop_env = "suburb"  # suffix dependning on whether urban or
            # suburban is considered
        elif fd_scen == 'scC_TSPIN_single_src':
            fd_dim = (100, 100)
            n_MC = 10
            n_sam = 1024
            n_src = 1
            ...
            sha_fa = True
            # Do we use a field with fast fading?
            fast_fa = {'exist': True, 'type': 'Rayleigh', 'param': np.sqrt(2 / np.pi)}
            ...
            # Do we use a field with shadow fading?
            sha_fa = True
            fast_fa = {'exist': False, 'type': 'null', 'param': np.nan}
        ...
        with open(os.path.join(
            ...
            pickle.dump([fd_dim, n_MC, n_sam, n_src, ran_cen, ran_rad, ran_pre,
                         add_tra, pi0_des, sha_fa, fast_fa, prop_env], output, -1)
    return fd_dim, n_MC, n_sam, n_src, pi0_des, sha_fa, fast_fa, prop_env
